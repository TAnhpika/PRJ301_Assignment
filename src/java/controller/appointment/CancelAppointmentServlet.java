...
public class CancelAppointmentServlet extends HttpServlet {
        ...
        try {
            ...
            if (appointments != null) {
                for (model.Appointment ap : appointments) {
                    System.out.println("[DEBUG] Appointment: id=" + ap.getAppointmentId() + ", status=" + ap.getStatus()
    ...
    /**
     * Endpoint lấy danh sách slot cho staff đổi lịch (có logic che slot đã qua giờ
     * bắt đầu + 10 phút)
     */
    private void handleGetTimeSlotsForStaff(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (doctorIdParam == null || doctorIdParam.trim().isEmpty() || workDate == null
                    || workDate.trim().isEmpty()) {
            ...
            java.util.List<model.TimeSlot> availableSlots = dao.TimeSlotDAO.getAvailableSlotsByDoctorAndDate(doctorId,
                    workDate);
            ...
            for (int i = 0; i < availableSlots.size(); i++) {
                ...
                boolean isPast = localDate.equals(today) && slot.getStartTime().plusMinutes(10).isBefore(now); // xử lý
                                                                                                               // thêm +
                                                                                                               // thêm
                                                                                                               // 10p để
                                                                                                               // render
        ...
        if (idStr != null) {
            try {
                ...
                if (rescheduleFlag != null) {
                    ...
                    success = dao.AppointmentDAO.updateAppointmentStatusStatic(appointmentId,
                            dao.AppointmentDAO.STATUS_CANCELLED);
                    if (success) {
                        ...
                        String patientName = ap != null && ap.getPatientName() != null ? ap.getPatientName()
                                : "Khách hàng";
                        String dateTime = ap != null && ap.getWorkDate() != null ? ap.getWorkDate().toString() : "";
                        if (ap != null && ap.getStartTime() != null)
                            dateTime += " " + ap.getStartTime().toString();
                        String service = ap != null && ap.getServiceName() != null ? ap.getServiceName() : "";
                        util.EmailService.sendCancelAppointmentEmail(patientEmail, patientName, dateTime, service,
