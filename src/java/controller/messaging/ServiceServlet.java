...
public class ServiceServlet extends HttpServlet {
        ...
        try {
            switch (action) {
                ...
                case "detail":
                    ...
                    break;
                case "specialties":
                    handleListSpecialties(request, response);
                    break;
                default:
                    handleListServices(request, response);
                    break;
    ...
    /**
     * Trả về JSON danh sách chuyên khoa (Specialties) - dùng cho dropdown khi thêm/sửa dịch vụ.
     */
    private void handleListSpecialties(HttpServletRequest request, HttpServletResponse response)
            throws IOException {
        response.setContentType("application/json");
        response.setCharacterEncoding("UTF-8");
        List<Specialty> specialties = specialtyDAO.getAllSpecialties();
        PrintWriter out = response.getWriter();
        out.print(gson.toJson(specialties));
        out.flush();
    }
    ...
    private void handleCreateService(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (specialtyIdStr != null && !specialtyIdStr.trim().isEmpty()) {
                try {
                    int sid = Integer.parseInt(specialtyIdStr.trim());
                    if (sid > 0) service.setSpecialtyId(sid);
